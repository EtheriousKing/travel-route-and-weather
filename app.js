...
app.post("/getTravelWeather", async (req,res) => {
    try {
        ...
        console.log(coordinates.data.resourceSets[0].resources[0].routeLegs[0]);
        res.render("index.ejs",{map : buffer64, location : coordinates.data.resourceSets[0].resources[0].routeLegs[0] });
    } catch (error) {
        console.error(error);
    }
})
